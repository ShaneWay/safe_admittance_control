...
{
  public:
    explicit KikuuweController(const std::string& config_file, DataLogger& logger) : BaseController(config_file, logger) {}
    ...
    Eigen::Vector2d getTorqueOnForceControl(Eigen::Vector2d& f_ext_from_sensor, Eigen::Vector2d& q_from_sensor) override
    {
        ...
        tau_star = Mat * (q_x_star - q_s_star);
    ...
    Eigen::Vector2d getTorqueOnPositionControl(Eigen::Vector2d& f_ext_from_sensor, Eigen::Vector2d& q_from_sensor) override
    {
        ...
        q_s_star = q_s + Mat2.inverse() * (phi_b - phi_a);

        tau_star = Mat2 * Mat1.inverse() * q_x_star - Mat2 * q_s_star;

        tau = proj(tau_star);

        q_x = q_s_star + Mat2.inverse() * tau;
        q_x_hat = (q_x - q_x_last) / dt;
        a = a_last + dt * (q_x - q_s);
