    ...
    try
    {
        ...
        while (timer_count < (15000))
        {
            ...
            if (now - begin > 2000)
            {
                ...
                cout << "f_input: " << f_input.transpose() << endl;
                ...
                // if(tau[0] + g[0] > 0)
                // {
                // base_command.mutable_actuators(0)->set_torque_joint(tau[0] + g[0] + 0.5);
                // }
                // else if(tau[0] + g[0] < 0)
                // {
                // base_command.mutable_actuators(0)->set_torque_joint(tau[0] + g[0] - 0.5);
                // }else
                // {
                //     base_command.mutable_actuators(0)->set_torque_joint(tau[0] + g[0]);
                // }

                // if(tau[1] + g[3] > 0)
                // {
                // base_command.mutable_actuators(3)->set_torque_joint(tau[1] + g[3] + 0.5);
                // }
                // else if(tau[1] + g[3] < 0)
                // {
                // base_command.mutable_actuators(3)->set_torque_joint(tau[1] + g[3] - 0.5);
                // }else
                // {
                //     base_command.mutable_actuators(3)->set_torque_joint(tau[1] + g[3]);
                // }
