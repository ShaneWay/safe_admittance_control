...
void getgrav(RigidBodyDynamics::Model* model, const VectorNd& q,
            VectorNd& grav)
{

    MatrixNd M = MatrixNd::Zero(model->qdot_size, model->qdot_size);
    CompositeRigidBodyAlgorithm(*model, q, M, false);

    VectorNd ZeroQdot = VectorNd::Zero(model->qdot_size);

    InverseDynamics(*model, q, ZeroQdot, ZeroQdot, grav);
...
int main (int argc, char* argv[]) {
	...
	unsigned int body_link1_id, body_link2_id,
    ...
    body_link3_id = model->AddBody(body_link2_id, SpatialTransform(trans_rotate3, trans_trans3), joint_3, body_3);
    ...
    body_link4_id = model->AddBody(body_link3_id, SpatialTransform(trans_rotate4, trans_trans4), joint_4, body_4);
    ...
    body_link5_id = model->AddBody(body_link4_id, SpatialTransform(trans_rotate5, trans_trans5), joint_5, body_5);
    ...
    body_link6_id = model->AddBody(body_link5_id, SpatialTransform(trans_rotate6, trans_trans6), joint_6, body_6);
    ...
    body_link7_id = model->AddBody(body_link6_id, SpatialTransform(trans_rotate7, trans_trans7), joint_7, body_7);
    ...
    auto posistion = CalcBodyToBaseCoordinates(*model, Q, body_link7_id, Vector3d(0, 0, -0.061525));
    ...
    CalcPointJacobian(*model, Q, 7, Vector3d(0, 0, -0.061525), jacobian);
    std::cout << "jacobian: \n" <<  jacobian << std::endl;
